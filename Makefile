...
help:
	...
	@echo "  make build                      - Build rrsh for all architectures"
	@echo "  make build-linux-amd64          - Build rrsh (Linux, amd64 only)"
	...
	@echo "  make install-linux-amd64        - Copy amd64 binary from dist/ to /usr/bin/rrsh"
	@echo "  make install-linux-armv6        - Copy armv6 binary from dist/ to /usr/bin/rrsh"
	@echo "  make install-linux-armv7        - Copy armv7 binary from dist/ to /usr/bin/rrsh"
	@echo "  make install-linux-arm64        - Copy arm64 binary from dist/ to /usr/bin/rrsh"
...
build-linux-amd64: deps
	goreleaser build --snapshot --clean --id rrsh_linux_amd64

build-linux-armv6: deps
	goreleaser build --snapshot --clean --id rrsh_linux_armv6

build-linux-armv7: deps
	goreleaser build --snapshot --clean --id rrsh_linux_armv7
...
install-linux-armv6: remove-binary
	sudo cp -a dist/rrsh_linux_armv6_linux_arm_6/rrsh /usr/bin/rrsh

install-linux-armv7: remove-binary
	sudo cp -a dist/rrsh_linux_armv7_linux_arm_7/rrsh /usr/bin/rrsh

install-linux-arm64: remove-binary
	sudo cp -a dist/rrsh_linux_arm64_linux_arm64/rrsh /usr/bin/rrsh

remove-binary:
	sudo rm -f /usr/bin/rrsh

install-linux-amd64-deb: purge-package
	sudo dpkg -i dist/rrsh_*_linux_amd64.deb

install-linux-armv6-deb: purge-package
	sudo dpkg -i dist/rrsh_*_linux_armv6.deb

install-linux-armv7-deb: purge-package
	sudo dpkg -i dist/rrsh_*_linux_armv7.deb

install-linux-arm64-deb: purge-package
	sudo dpkg -i dist/rrsh_*_linux_arm64.deb

purge-package:
	sudo apt-get purge rrsh || true

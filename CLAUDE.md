...
- Three methods: `hello` (name, version, instructions), `list` (allowlist), `run` (executes one `argv` or a `pipeline`).
...
| `server/` | JSON-RPC 2.0 server: NDJSON framing, three-method API (`hello`/`list`/`run`). `server/types.go` holds wire types + JSON-RPC error codes; `server/server.go` holds the dispatch loop and handlers. |
...
rrsh used to expose an MCP-compatible surface (`initialize`/`tools/list`/`tools/call`). It no longer does. The protocol is now plain JSON-RPC 2.0 over NDJSON with three methods: `hello`, `list`, `run`. Server-side refusals (matcher denial, elevation disabled, oversize pipeline) use the JSON-RPC `error` envelope with code `-32000`. A child process's own non-zero exit is **not** an RPC error — it lives in `result.exit`. See the README's "Wire format" section for the wire shape.

The binary version is populated by main from goreleaser's ldflags into `server.Version`, and surfaces in `hello.version`.
